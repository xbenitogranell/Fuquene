...
F7_pollen <- read.csv("data/Fuquene_F7_pollen.csv", sep = ";") %>%
  ...
  select(-c(115,116,117,118,119)) %>% #exclude spores 
  rename(depth=ï..depth)
...
range(pollen_long$upper_age) ## ULL here, need to transform to numeric
...
#make it wide--counts
core_counts_wide_F9 <- core_counts_common_F7 %>%
  select(depth, upper_age, lower_age, taxa, count) %>%
  spread(key = taxa, value = count) %>%
  arrange(upper_age)
...
## Join M1 and F9 records by common taxa names (columns)
...
# write results
write.csv(pollen_M1_F9, "outputs/pollen_M1_F9_counts.csv")
```
...
## Composite pollen M1 & M9 cores (>%5 relative abundance & counts)
```{r}
## Read in change nms table
nms_pollen <- read.csv("data/nms_changes_pollenM1M9.csv", sep = ";")
head(nms_pollen)

## Change pollen names and groups
pollen_M1_F9 <- read.csv("outputs/pollen_M1_F9_ra.csv")[-1] %>% #remove column core
  gather(key = taxa, value = prop, -depth, -upper_age, -lower_age, -core) %>%
...
ggplot(pollen_M1_M9, aes(y=upper_age, x=Norm_sqrt_pc)) +
  geom_line(orientation="y") +
  facet_grid(. ~ group, scales="free_x", space="free_x") +
  scale_x_continuous(breaks=seq(0, 100, by=20)) +
  scale_y_reverse(breaks=seq(0, 284000, by=1000)) +
  labs(x="%", y="Years BP")
...
## Change pollen names and groups
pollen_longdata <- read.csv("outputs/pollen_M1_F9_counts.csv")[-1] %>% #remove column core
  gather(key = taxa, value = count, -depth, -upper_age, -lower_age, -core) %>%
  mutate(taxa = plyr::mapvalues(taxa, from = nms_pollen[,1], to = nms_pollen$new)) %>%
  #mutate(group = plyr::mapvalues(taxa, from = nms_pollen[,2], to = nms_pollen$habitat)) %>%
  group_by(depth) %>%
  ...
  ungroup() %>%
  group_by(depth, taxa) %>%
  filter(relative_abundance_percent > 10) %>%
  filter(!is.na(elapsedTime)) %>%
  mutate(spp = factor(taxa)) %>%
  ungroup() 
...
pollen_gam_S <- gam(count ~ s(negAge, spp, k=20, bs="fs") + offset(log_total_counts),
                    weights = elapsedTime/mean(elapsedTime),
                    data=pollen_longdata, family = nb, #places notes at the deciles of sample ages
                    method = "REML")
...
pollen_plot_data <- with(pollen_longdata, as_tibble(expand.grid(negAge = seq(min(diat_data$negAge), max(diat_data$negAge)),
                                                        spp = factor(levels(pollen_longdata$spp)),
                                                        log_total_counts = mean(log_total_counts))))
...
pollen_plot <- ggplot(pollen_plot_data) +
  ...
  geom_ribbon(aes(x=negAge,
                  ...
                  fill = model),
              alpha=0.2)+
  geom_point(data= pollen_longdata, aes(x = negAge, y = count), size=0.06) +
  geom_line(aes(x = negAge, y = fit, color = model))+
  labs(y = "Absolute counts", x = "Age (cal yr BP)") +
  scale_fill_brewer(name = "", palette = "Dark2",
...
## save derivative summaries for later use
write.csv(deriv_summaries, "outputs/pollen-derivatives.csv", row.names = FALSE)
```
