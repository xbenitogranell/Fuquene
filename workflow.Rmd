...
# write results
write.csv(pollen_M1_F7_F9, "outputs/pollen_M1_F7_F9_counts.csv")
```
...
## Composite pollen M1, F7 & F9 cores (>%5 relative abundance & counts)
```{r}
## Read in change nms table
nms_pollen <- read.csv("data/nms_changes_pollen_M1_F7_F9.csv", sep = ",")
head(nms_pollen)
#colnames(nms_pollen)[1] <- "taxa"

## Change pollen names and groups
pollen_M1_F7_F9 <- read.csv("outputs/pollen_M1_F7_F9_counts.csv")[-1] %>% #remove column core
  gather(key = taxa, value = count, -depth, -upper_age, -lower_age, -core) %>%
...
pollen_plot <- ggplot(pollen_M1_F7_F9, aes(x = Norm_sqrt_pc, y = upper_age)) +
  geom_col_segsh() +
  #scale_y_reverse() +
...
```{r}
pollen_M1_F7_F9_wide <- read.csv("outputs/pollen_M1_F7_F9_counts.csv")[-1]
